...
class ProfileFragment : Fragment() {
    ...
    // ← CHANGED: removed refreshProfile() — Firestore listener handles it
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // ← CHANGED: removed tvUserId — Firebase UID not shown to user
        ...
        viewModel.userName.observe(viewLifecycleOwner) {
            tvName.text = it.ifBlank { "Reader" }
        }
        viewModel.savedAddress.observe(viewLifecycleOwner) {
            if (etAddress.text.toString() != it) etAddress.setText(it)
        }
